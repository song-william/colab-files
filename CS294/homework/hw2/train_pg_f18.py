...
class Agent(object):
    ...
    def sample_action(self, policy_parameters):
        ...
        if self.discrete:
            ...
            # YOUR_CODE_HERE
            sy_sampled_ac = tf.exp(sy_logstd) * tf.random_normal(tf.shape(sy_mean)) + sy_mean
    ...
    def build_computation_graph(self):
        ...
        #========================================================================================#
        # temp_prob = tf.Print(self.sy_logprob_n, [self.sy_logprob_n], "self.sy_logprob_n: {}".format(tf.shape(self.sy_logprob_n)))
        # temp_adv = tf.Print(self.sy_adv_n, [self.sy_adv_n], "self.sy_adv_n: {}".format(tf.shape(self.sy_adv_n)))
        temp = self.sy_logprob_n * self.sy_adv_n
        loss = -tf.reduce_mean(temp)
        # self.loss = -tf.reduce_mean(temp_prob * temp_adv)
        ...
        if self.nn_baseline:
            ...
            baseline_loss = 0.5 * tf.losses.mean_squared_error(self.sy_target_n, self.baseline_prediction)
    ...
    def sum_of_rewards(self, re_n):
        ...
        if self.reward_to_go:
            ...
            for rewards in re_n:
                ...
                sums = [0]*(path_len-1)
                sums.append(rewards[-1])
                for i in range(path_len-2, -1, -1):
                    sums[i] = (self.gamma * sums[i+1]) + rewards[i]
                q_n = np.concatenate((q_n, sums))

        else:
            q_n = np.array([])
            for rewards in re_n:
                reward_sum = sum([(self.gamma ** i)*r for i, r in enumerate(rewards)])
                q_n = np.concatenate((q_n, len(rewards) * [reward_sum]))
    ...
    def update_parameters(self, ob_no, ac_na, q_n, adv_n):
        ...
        if self.nn_baseline:
            ...
            c = self.sess.run([self.baseline_update_op], feed_dict={self.sy_target_n: target_n, self.sy_ob_no: ob_no})
        ...
        # YOUR_CODE_HERE
        c = self.sess.run([self.update_op], feed_dict={self.sy_ob_no: ob_no, self.sy_ac_na: ac_na, self.sy_adv_n: adv_n})
