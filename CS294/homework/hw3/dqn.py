...
class QLearner(object):
  ...
  def __init__(
    ...
    ######
    # YOUR CODE HERE
    q_values = q_func(obs_t_float, self.num_actions, scope="q_func")
    target_q_values = q_func(obs_tp1_float, self.num_actions, scope="target_q_func")
    y = self.rew_t_ph + gamma * tf.reduce_max(target_q_values, axis=1) * (1 - self.done_mask_ph)

    # index q values with the actions the agent actually took
    range_idxs = tf.range(tf.shape(q_values)[0])[:, tf.newaxis]
    actions = self.act_t_ph[:, tf.newaxis]
    idxs = tf.concat([range_idxs, actions], axis=1)
    actual_q_values = tf.gather_nd(q_values, idxs)

    self.total_error = tf.reduce_sum(huber_loss(actual_q_values - y))
    q_func_vars = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope='q_func')
    target_q_func_vars = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope='target_q_func')
    self.action = tf.argmax(q_values, axis=1)
    ######
  ...
  def step_env(self):
    ...
    # we scale epislon because randint still has 1/num_actions chance of selecting the optimal action
    should_explore = random.random() < self.exploration.value(self.t) * self.num_actions / (self.num_actions - 1)
    if not self.model_initialized or should_explore:
        ac = random.randint(0, self.num_actions - 1)
    else:
        obs = self.replay_buffer.encode_recent_observation()
        ac = self.session.run(self.action, feed_dict={self.obs_t_ph: obs[None]})[0]

    ob, reward, done, info = self.env.step(ac)
    self.replay_buffer.store_effect(self.replay_buffer_idx, ac, reward, done)
    self.last_obs = ob
    if done:
        self.last_obs = self.env.reset()
  ...
  def update_model(self):
    ...
    if (self.t > self.learning_starts and \
        ...
        # YOUR CODE HERE
        obs_t, acts, rews, obs_tp1, done_mask = self.replay_buffer.sample(self.batch_size)
