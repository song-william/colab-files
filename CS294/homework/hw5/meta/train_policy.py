...
def build_rnn(x, h, output_size, scope, n_layers, size, activation=tf.tanh, output_activation=None, regularizer=None):
    ...
    #====================================================================================#
    # YOUR CODE HERE
    x = build_mlp(x, size, scope, n_layers, size, activation=activation, output_activation=output_activation, regularizer=regularizer)
    cell = tf.keras.layers.GRUCell(output_size, activation=activation)
    x, h = tf.nn.dynamic_rnn(cell, x, initial_state=h)
    ...
    def sample_trajectory(self, env, animate_this_episode, is_evaluation):
        ...
        while True:
            ...
            if ep_steps == 0:
                ...
                # YOUR CODE HERE
                a, r, d = np.zeros(self.ac_dim), np.zeros(self.reward_dim), np.zeros(self.terminal_dim)
                meta_obs[steps] = np.concatenate((ob, a, r, d))
                steps += 1
            ...
            # YOUR CODE HERE
            in_ = meta_obs[max(steps-self.history, 0):steps]
            in_ = np.pad(in_, ((self.history - len(in_),0),(0,0)), "constant")
