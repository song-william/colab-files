...
class Histogram(Density_Model):
    ...
    def get_count(self, states):
        ...
        """
        counts = [self.hist[self.preprocessor(s)] for s in states]
        return counts
...
class RBF(Density_Model):
    ...
    def get_prob(self, states):
        ...
        if self.means is None:
            ...
            # 1. Compute deltas
            deltas = np.array([s - self.means for s in states])
            ...
            # Gaussian
            gaussians = np.exp(-1 * euc_dists / (2 * self.sigma**2))
...
class Exemplar(Density_Model):
    ...
    def update(self, state1, state2, target):
        ...
        ll, kl, elbo = self.sess.run([self.log_likelihood, self.kl, self.elbo], feed_dict=feed_dict)
        return ll, kl, elbo
